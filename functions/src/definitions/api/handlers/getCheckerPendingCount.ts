import { Request, Response } from "express"
import * as admin from "firebase-admin"
import { logger } from "firebase-functions/v2"

if (!admin.apps.length) {
  admin.initializeApp()
}

const db = admin.firestore()

const getCheckerPendingCount = async (req: Request, res: Response) => {
  try {
    const checkerId = req.params.checkerId
    if (!checkerId) {
      return res.status(400).send("Checker ID missing.")
    }
    const checkerDocRef = db.collection("checkers").doc(checkerId)
    let query = db
      .collectionGroup("voteRequests")
      .where("factCheckerDocRef", "==", checkerDocRef)
      .where("category", "==", null)
      .count()
    const pendingCount = (await query.get()).data().count
    return res.status(200).send({ pendingCount })
  } catch (error) {
    logger.error(error)
    return res.status(500).send("Internal server error")
  }
}

export default getCheckerPendingCount
