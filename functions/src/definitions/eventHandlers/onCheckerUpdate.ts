  ...
  async (event) => {
    ...
    if (postProgramEnd && preProgramEnd !== postProgramEnd) {
      try {
        ...
        if (!userName) {
          logger.error(`User name is missing for user ID ${userId}. Certificate generation aborted.`);
        ...
        if (!exists) {
          ...
          logger.info(`Entering generateAndUploadCertificate for user ID: ${userId}, userName: ${userName}, programEnd: ${postProgramEnd}`);

          const certificateUrl = await generateAndUploadCertificate(userId, userName, postProgramEnd);
          ...
          logger.info(`Certificate already exists in bucket for user ID: ${userId}. Skipping generation.`);
        }
      } catch (error) {
        logger.error(`Error on checker update for ${postChangeSnap.id}: ${error}`);
      }
    }
