const { sendWhatsappTextMessage, sendWhatsappImageMessage } = require("./sendWhatsappMessage");
...
exports.sendImageMessage = async function (bot, to, urlOrId = null, caption = null, replyId = null, platform = "whatsapp") {
  let res;
  switch (platform) {
    case "telegram":
      if (checkUrl(urlOrId)) {
    ...
    case "whatsapp":
      if (checkUrl(urlOrId)) {
        res = await sendWhatsappImageMessage(bot, to, null, urlOrId, caption, replyId);
      } else {
        res = await sendWhatsappImageMessage(bot, to, urlOrId, null, caption, replyId);
      }
      break
  }
  return res;
