exports.USER_BOT_RESPONSES = {
  ...
  IRRELEVANT: "Thank you for waiting! Our CheckMates have reviewed the message and think it's *harmless*.👌 Still, such messages could add workload for our CheckMates, so we'd appreciate if you try and avoid sending in trivial messages in the future!",
  IRRELEVANT_AUTO: "Thank you for waiting! This message is likely *harmless*.👌 Still, such messages could add workload for our CheckMates, so we'd appreciate if you try and avoid sending in trivial messages in the future!",
  ...
  SPAM: "🚧🚧 Thank you for waiting! Our CheckMates have reviewed this message and think this looks like spam! It's likely harmless, but you should always make sure!\n\nThank you for keeping Singapore safe!",
  LEGITIMATE: "✅✅Thank you for waiting! Our CheckMates have reviewed the message and think that it's *from a legtimate source*.✅\n\nThank you for keeping Singapore safe!",
  UNSURE: "🤷🏻‍♂️🤷🏻‍♀️Thank you for waiting! Unfortunately, our CheckMates are *unsure about this message*.😞 Sorry about that, we're human too! If you haven't already done so, you could try sending in the message with more context, for example by sending in a screenshot that contains the sender's number instead.\n\nThank you for keeping Singapore safe!",
